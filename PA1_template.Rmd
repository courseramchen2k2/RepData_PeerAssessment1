...
```
The mean is `r CalMean` and the median is `r CalMedian` for the total number of steps taken per day.
...
occured in interval `r MaxInterval` which is `r Time` in the morning.
...
Days <- RowNA / Interval
```
There are total of `r RowNA` rows with NA's and since each interval is 5 mins
which makes `r Interval` intervals in a day and `r Days` days worth of NA values. 
...
```
The mean is `r CalMean2` and the median is `r CalMedian2` for the total number of steps taken per day after the missing values are imputed.
